...
Handle<Value> wrap_file_pointer(FILE* fp)
{
...
FILE* unwrap_file_pointer(Handle<Object> self)
{
...
/*
	METHODS
*/
Handle<Value> js_io_new(const Arguments& args)
{
...
Handle<Value> js_io_open(const Arguments& args)
{
	if (args.Length() < 1)
...
Handle<Value> js_io_close(const Arguments& args)
{
...
Handle<Value> js_io_read(const Arguments& args)
{
	FILE* fp = unwrap_file_pointer(args.This());
...
Handle<Value> js_io_write(const Arguments& args)
{
	FILE* fp = unwrap_file_pointer(args.This());
...
Handle<Value> js_io_flush(const Arguments& args)
{
	FILE* fp = unwrap_file_pointer(args.This());
	return fflush(fp) ? False() : True();
}

Handle<Value> js_io_seek(const Arguments& args)
{
	FILE* fp = unwrap_file_pointer(args.This());
...
Handle<Value> js_io_tell(const Arguments& args)
{
	FILE* fp = unwrap_file_pointer(args.This());
...
Handle<Value> js_io_handle(Local<String> property, const AccessorInfo& info)
{
	return Integer::New((int64_t)unwrap_file_pointer(info.Holder()));
}

Handle<Value> js_io_eof(Local<String> property, const AccessorInfo& info)
{
...
void dummy_setter(Local<String>, const Local<Value>, const AccessorInfo&) {}
